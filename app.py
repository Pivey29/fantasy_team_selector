...
def load_player_data():
    # Path must match your GitHub structure (data/test.csv)
...
# Team Limit Tracking (Max 2 per team)
...
if st.session_state.submitted:
    st.success("🎉 Submission received! Good luck.")
    ...
    with current_tab:
        ...
        for _, row in disp_df.iterrows():
            ...
            if is_in:
                ...
                # Add logic including Team Limit
...
if valid_size:
    if not manager_name:
        ...
        st.warning(f"⚠️ Gender balance required (4/5 or 5/4 split).")
    elif not has_caps:
        st.warning("⚠️ You must pick one Captain from each division in the sidebar.")
