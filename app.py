...
@st.cache_data(ttl=3600)  # High TTL since you said data won't change
def load_player_data():
    try:
        ...
        # Sort by price descending for a clean UI
...
if st.session_state.submitted:
    st.success("🎉 Team submitted successfully! Good luck.")
...
manager_name = st.text_input("Manager Name:", key="mgr_name_persistent", placeholder="Type name & surname and press Enter...").strip()
...
if valid_size:
    if not manager_name:
        ...
        st.warning("⚠️ You must pick a Captain for each division in the sidebar or list.")
        ...
        if st.button("🚀 SUBMIT FINAL TEAM", use_container_width=True):
            with st.spinner("Connecting to league records..."):
                try:
                    # 1. Fetch fresh data to ensure we append to the absolute end
                    ...
                    if "429" in str(e):
                        ...
                        st.error(f"Error: {e}. If this persists, screenshot your roster!")
else:
